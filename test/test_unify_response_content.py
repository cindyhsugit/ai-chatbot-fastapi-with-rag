import app.utility.unify_response_content


def test_to_text_plain_string():
    # OpenAI-style response: content is already a plain string
    result = app.utility.unify_response_content.to_text(
...
def test_to_text_list_of_dicts_gemini_shape():
    ...
    result = app.utility.unify_response_content.to_text(content)
...
def test_to_text_non_str_non_list_input():
    ...
    result = app.utility.unify_response_content.to_text(None)
    assert result == "None"

    result = app.utility.unify_response_content.to_text(123)
    assert result == "123"
